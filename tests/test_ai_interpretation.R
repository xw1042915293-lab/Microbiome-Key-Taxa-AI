...
options(kkai.report_language = "en")
md_en <- build_diff_interpretation(job_dir)
options(kkai.report_language = "zh")
md_zh <- build_diff_interpretation(job_dir)
options(kkai.report_language = NULL)

stopifnot(grepl("No FDR-significant taxa were detected", md_en))
stopifnot(grepl("Top exploratory taxa", md_en))
stopifnot(grepl("Exploratory trends are hypothesis-generating and require experimental validation\\.", md_en))
stopifnot(!grepl("Mean abundance was", md_en, fixed = TRUE))
stopifnot(!grepl("prevalence was", md_en, fixed = TRUE))
stopifnot(grepl("未检测到 FDR < 0.05 的显著差异菌", md_zh, fixed = TRUE))
stopifnot(grepl("## 探索性趋势菌", md_zh, fixed = TRUE))
stopifnot(grepl("## 潜在生物学意义", md_zh, fixed = TRUE))
stopifnot(grepl("## 综合筛选建议", md_zh, fixed = TRUE))
stopifnot(grepl("## 结果解释限制", md_zh, fixed = TRUE))
