  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    scan_jobs <- function() {
      ...
      rows <- lapply(job_dirs, function(job_dir) {
        ...
        data.frame(
          ...
          table_count = count_files(file.path(job_dir, "tables")),
          figure_count = count_files(file.path(job_dir, "figures")),
    ...
    output$jobs_table <- DT::renderDT({
      ...
      tbl <- data.frame(
        ...
        group_var = ifelse(is.na(df$group_var) | !nzchar(df$group_var), "-", df$group_var),
        tax_level = ifelse(is.na(df$tax_level) | !nzchar(df$tax_level), "-", df$tax_level),
        report = vapply(df$report_exists, bool_badge_html, character(1)),
        key_taxa = vapply(df$key_taxa_score_exists, bool_badge_html, character(1)),
        ml = vapply(df$ml_exists, bool_badge_html, character(1)),
        network = vapply(df$network_exists, bool_badge_html, character(1)),
        status = vapply(df$status, status_badge_html, character(1)),
        stringsAsFactors = FALSE
      )
      names(tbl) <- c("任务ID", "创建时间", "分组变量", "分类层级", "报告", "KeyTaxa", "机器学习", "网络", "状态")
    ...
    output$checks_table <- DT::renderDT({
      ...
      df <- data.frame(
        ...
        status = vapply(
          core_files,
          function(p) {
            if (file.exists(file.path(sj$job_dir[[1]], p))) {
              badge_html("已生成", "kkai-badge kkai-badge--done")
            } else {
