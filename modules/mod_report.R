  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$dl_key_tables <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        rel <- c(
          ...
          "tables/ml_cross_validation_metrics.csv",
          "tables/ml_out_of_fold_predictions.csv",
          "tables/ml_confusion_matrix.csv",
          "tables/ml_top_taxa_abundance_statistics.csv",
          "tables/ml_permutation_test.csv",
    ...
    output$dl_figures <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        alpha_figs_dir <- file.path(job$job_dir, "alpha", "figures")
        beta_figs_dir <- file.path(job$job_dir, "beta", "figures")
        rel <- character(0)
        if (dir.exists(figs_dir)) {
          rel <- file.path("figures", list.files(figs_dir, recursive = TRUE, full.names = FALSE, no.. = TRUE))
        }
        if (dir.exists(alpha_figs_dir)) {
          rel <- c(rel, file.path("alpha", "figures", list.files(alpha_figs_dir, recursive = TRUE, full.names = FALSE, no.. = TRUE)))
        }
        if (dir.exists(beta_figs_dir)) {
          rel <- c(rel, file.path("beta", "figures", list.files(beta_figs_dir, recursive = TRUE, full.names = FALSE, no.. = TRUE)))
        }
        res <- safe_zip_selected_files(
          base_dir = job$job_dir,
          rel_files = rel,
