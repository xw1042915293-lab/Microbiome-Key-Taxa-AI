# Beta 结果页：预览 job_dir/figures 下的 PCoA 图。

mod_beta_ui <- function(id) {
  ns <- shiny::NS(id)
  shiny::fluidPage(
    shiny::h3("Beta 多样性"),
    shiny::p("展示 Bray-Curtis PCoA 图，图像已保存到任务目录中。"),
...
mod_beta_server <- function(id, state) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$plot_ui <- shiny::renderUI({
      ...
      png_file <- "beta_pcoa_bray.png"
      full <- file.path(state$job_dir, "figures", png_file)
      if (!file.exists(full)) return(shiny::helpText("尚未生成 Beta 图。"))
      bslib::card(
        class = "kkai-card",
        bslib::card_header("Beta 多样性图"),
